function tm --description 'Manage Transmission-CLI services and magnet links'
    ...
    # -G (not -w) is the connect timeout on macOS's BSD nc: -w is only an idle
    # timeout for an already-open connection and doesn't bound connect() at all.
    set -l host_ip (string split ':' $host)[1]
    set -l host_port (string split ':' $host)[2]
    if not nc -z -G 1 $host_ip $host_port >/dev/null 2>&1
