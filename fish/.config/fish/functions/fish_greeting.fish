function fish_greeting
    ...
    if set -q GHOSTTY_RESOURCES_DIR
        # Walk ancestry to find the Ghostty process PID, which is stable across all tabs in a window
    ...
    if test -n "$terminal_key"; and test "$fish_last_greeted_pid" != "$terminal_key"
