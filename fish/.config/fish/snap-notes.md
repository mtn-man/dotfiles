...
Dotfiles are also backed up to a remote github repo and symlinked into place with GNU stow.
A CentOS Stream 10 homelab is accessible over Tailscale, with SMB shares mounted on demand
via Finder.
...
kitty is kept installed for its kitten icat image rendering;
Ghostty is my primary terminal emulator.
...
ctrl+shift+e = eject all external disks -- via Supercharge
...
meta+d = search DuckDuckGo (my preferred engine)
meta+j = open Jellyfin web client in browser
